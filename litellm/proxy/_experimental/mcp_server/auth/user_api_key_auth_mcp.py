...
class MCPRequestHandler:
        ...
        try:
            ...
            for source in await MCPRequestHandler._admitted_subject_sources(auth):
                ...
                if not isinstance(team_limit, dict) or not team_limit:
                    continue
                granted_ids = set(await MCPRequestHandler.get_allowed_mcp_servers(source, keyless_source=True))
                applicable = {
                    server_name: rpm
    ...
    async def _resolve_admitted_subject_servers(auth: UserAPIKeyAuth) -> list[str]:
        ...
        for source in await MCPRequestHandler._admitted_subject_sources(auth):
            reachable.update(await MCPRequestHandler.get_allowed_mcp_servers(source, keyless_source=True))
    ...
    async def _resolve_admitted_subject_tools(server_id: str, auth: UserAPIKeyAuth) -> list[str] | None:
        ...
        allowed: set[str] = set()
        for source in await MCPRequestHandler._admitted_subject_sources(auth):
            if server_id not in await MCPRequestHandler.get_allowed_mcp_servers(source, keyless_source=True):
                continue
    ...
    async def get_allowed_tools_for_server(
        ...
        *,
        ...
        try:
            ...
            # server, each answered by this same function, so a team's (and its org's)
            # mcp_tool_permissions bind exactly as they would for a key scoped to that team.
